...
## Hardening, Scale Prep & Public Launch — status (implementationplan.md Group K — complete, phases 94–100)

Cross-cutting and fundamentally different in character from every group above: not new screens/schema, but auditing, load-testing, and incident-readiness work against everything Groups A–J already built. Each phase's deliverable type was matched honestly to what this environment can actually do — real, executable audits and drills where the infrastructure allows, clearly-labeled scoping documents (not fabricated results) where a phase needs resources this session doesn't have (a device farm, a real Supabase cloud region, legal counsel, app store accounts). Two real, safety-critical bugs were found and fixed along the way, plus two app-store-readiness misconfigurations — full detail in `docs/`.

**Phase 94 — RLS Full Isolation Sweep** (`supabase/tests/rls_full_isolation_sweep_test.sql`, edgecase.md §11.1 final closure): unlike Phase 5's `rls_isolation_harness_test.sql` (which proves the *pattern* on a disposable schema), this queries the real app's system catalogs directly — every table has RLS enabled, every RLS-enabled table has a real policy, no policy unconditionally exposes rows, every INSERT policy has a genuine `WITH CHECK`, `anon` has zero grants anywhere, and audit-trail/ledger tables (`sos_dispatch_log`, `point_events`, etc.) grant no `DELETE` to `authenticated` — all six checks pass against the actual 40-table schema today, and (because they're queries against the catalog, not a fixed list) will keep checking automatically as future tables get added.

**Phase 95 — Realtime Scale/Load Testing** (`docs/realtime-scale-load-assessment.md`): code-reviewed all 3 real `.channel()` subscriptions (chat messages, SOS dispatch, safety alerts) for connection-leak bugs — none found, all clean up correctly on unmount. Honestly flags that this session's Docker-free stack has no Realtime server component at all, so the websocket layer itself was never live-load-tested here (only Supabase Cloud would have one) — a real, named gap, not glossed over. Executed instead: a real 200-concurrent-request test against the underlying Postgres/PostgREST connection pool (200/200 succeeded, sub-30ms typical latency on a 10-connection pool), plus a scoped (per the plan's own "not necessarily executed" wording) Ably/Pusher migration path.

**Phase 96 — Low-End Device Performance Testing** (`docs/low-end-device-performance-audit.md`): audited every data-driven screen for list-virtualization correctness — every genuinely unbounded list already uses `FlatList`/`SectionList` (18 files), a clean result. Real, executed bundle-size measurement via `expo export --platform web` (5.7MB / 1.13MB gzipped), honestly caveated as a web-bundle proxy, not real Hermes/native Android numbers — no physical low-end device or emulator exists in this environment, named as a real gap with a concrete recommended sweep procedure for whoever has the hardware.

**Phase 97 — Network-Degradation Testing** (`docs/network-degradation-test-results.md`) — fully executable, and found real bugs: killed the entire backend and tested SOS + verification live. Found and fixed **(1)** Circle Guard's SOS button could become permanently unpressable offline (`getUser()`'s network round-trip blocking `userId`, switched to local-only `getSession()`); **(2)** the native `tel:` emergency dial used to be sequenced behind two network-dependent steps, exactly backwards from edgecase.md §3.1's "must work with zero connectivity" — fixed by generating the SOS event id client-side (`expo-crypto`) so the dial fires first, unconditionally, before anything network-dependent; **(3)**, found incidentally while re-testing: `RootNavigator`'s onboarding gate conflated "request failed" with "not onboarded," which would have silently routed a fully-verified offline user back through the entire Address/live-selfie gauntlet — fixed to assume onboarded on a network error rather than route the whole app on a question that never got answered. All three fixes were live-re-verified against the real running app with the backend still fully dead.

**Phase 98 — Data Residency/Compliance Final Check** (`docs/data-residency-compliance-check.md`): not legal advice, same discipline as Phase 56's SOS legal-review doc — confirms the Mumbai-region decision (architecture.md §9), tables every SPDI-adjacent column this build actually stores, and checklists DPDP Act 2023 alignment (consent, purpose limitation, data-principal rights, breach notification, localization) against what's built today, naming two concrete engineering gaps (an itemized consent screen, a self-service data-export feature) for whenever counsel gives specifics.

**Phase 99 — App Store Submission Prep** (`docs/app-store-submission-prep.md`, edgecase.md §11.3 🔴 — the plan's own highest-severity item in this group): real pre-validation, not a promise to check later. Found and fixed two real `app.json` misconfigurations: the microphone permission string described only "voice notes in chats," silently omitting Silent Phrase's wake-word listening (Phase 54) — both features share one OS-level mic permission, so the string now discloses both; and the location permission was configured for iOS "Always" background access the app never actually requests (every real call site uses `requestForegroundPermissionsAsync` only) — narrowed to `locationWhenInUsePermission`, removing unnecessary background-location review scrutiny for a capability that doesn't exist. A full store-listing checklist covers what's left (real accounts, screenshots, ratings questionnaire) once those external resources exist.

**Phase 100 — Incident-Response Runbooks + Game Day Drill** (`docs/runbooks/{rls-leak,sos-dispatch-failure,payment-failure}.md`, `docs/game-day-drill-log.md`): three real runbooks, each grounded in this repo's actual tables/functions rather than generic advice. One live drill executed (not a tabletop exercise): killed `services/sos` specifically (everything else healthy) and triggered a real SOS event. Result: the native `tel:` dial succeeded regardless (proving Phase 97's fix holds under this failure shape too), the backend fan-out failure surfaced honestly in the UI ("Failed to fetch," not a false success), and running the runbook's own triage query against the real incident correctly diagnosed the root cause in one step — `sos_dispatch_log` showed exactly 3 `tel:` rows and zero `trusted_contact`/`nearby_neighbour` rows, the documented signature for "backend fan-out never ran," not a partial/degraded failure.

RLS: `supabase/tests/rls_full_isolation_sweep_test.sql` (6 assertions) — 125 total across the whole suite, all passing on a full clean-slate migration replay. Same Docker-free verification environment as Groups I–J (see the note above).
