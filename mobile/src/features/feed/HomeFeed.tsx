...
// needed, the database only returns what's visible), with hidden posts and
// muted authors filtered client-side.
export default function HomeFeed() {
  ...
  const load = useCallback(async () => {
    ...
    const [{ data: rows }, { data: hidden }, { data: muted }] = await Promise.all([
      supabase
        .from('posts')
    ...
    const mapped: FeedPost[] = (rows ?? [])
      .filter((r) => !hiddenIds.has(r.id) && !mutedIds.has(r.author_id))
      .map((r) => {
